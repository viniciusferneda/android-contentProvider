...
class NotesAdapter(private val listener: NoteClickedListener) :
    ...
    override fun onBindViewHolder(holder: NotesViewHolder, position: Int) {
        ...
        holder.noteTitle.text =
            mCursor?.getString(mCursor?.getColumnIndex(NotesDatabaseHelper.TITLE_NOTES) as Int)
        holder.noteDescription.text =
            mCursor?.getString(mCursor?.getColumnIndex(NotesDatabaseHelper.DESCRIPTION_NOTES) as Int)
